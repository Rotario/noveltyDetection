...
inline float SVM_predictEEPROM(float* sensor, uint8_t nSensor) {
  ...
  for (i = 0; i < svm.nVecDims; i++) {
    char buf[50];
    snprintf(50, buf, "Scaled Values[%i]: %.5f\n", i, scaledSensor[i]);
    Serial.print(buf);
  }

  //One class implementation
  float sv[SVM_MAX_VEC_DIM];
  float sum = 0;

  //Iterate through ALL Support vectors
  for (int i = 0; i < svm.totalSVs; i++) {
    float sv_coef;
    ...
    if (f) {
      ...
      while (f.available()) {
	    ...
	    Serial.print(atof(pch));
            ...
            for (int i = 0; i < svm.nVecDims; i++) { //should always be less than 10. if not then never gets to this point, returns in svm.mod file parsing stage
              readToCharCode(&f, 0x0A, SD_BUF_LENGTH, sdBuf); //Fill messagebuffer with file until LF
